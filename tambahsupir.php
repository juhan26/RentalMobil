...
// Cek apakah ini adalah halaman edit
$is_edit = isset($_GET['id']);
if ($is_edit) {
    $supir_id = $_GET['id'];
    // Ambil data supir dari database dengan prepared statement
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($alamat) || empty($tanggal_lahir) || empty($telp)) {
        ...
        // Cek nomor telepon hanya angka dan panjangnya sesuai kebutuhan
        if (!preg_match('/^\d{10,12}$/', $telp)) {
            ...
            if ($result_check_telp && mysqli_num_rows($result_check_telp) > 0) {
                ...
                if ($is_edit) {
                    // Update data supir dengan prepared statement
                    ...
                    // Tambah data supir baru dengan prepared statement
                }
            }
        }
    }
            ...
            <div id="content-wrapper">
                <div class="card shadow-sm rounded">
                    <div class="card-body">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?><?php echo $is_edit ? '?id=' . $supir_id : ''; ?>" method="POST">
